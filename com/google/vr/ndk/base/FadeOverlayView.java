...
class FadeOverlayView extends View {
    ...
    public void updateFade() {
        ...
        if (currentAnimationTimeMillis >= this.fadeDurationMillis || ((i = this.fadeType) != 1 ? !(i != 2 || min < 1.0f) : min <= 0.0f)) {
            endFade();
        } else {
    ...
    public void onVisible() {
        ...
        if (!isEnabled() || !this.autoFadeEnabled) {
            ...
            removeFadeCallbacks();
            this.autoFadeHandler.sendEmptyMessageDelayed(77337733, true != this.flushAutoFadeOnVisible ? 1000L : 200L);
        }
    }
